...
# Typical invocation:
# spark-submit 1-osm-tourism.py /courses/datasets/openstreetmaps tourism
...
def get_tourism(line):
    ...
    if 'name' in tags:
        ...
        name = None
    yield Row(lat=lat, lon=lon, unix_time=unix_time, tourism=tourism, name=name, tags=tags)


def main(inputs, output):
    lines = sc.textFile(inputs)
    nodes = lines.flatMap(get_tourism)
    tourism = spark.createDataFrame(nodes, schema=tourism_schema)
    # work around Python to Spark datetime conversion problems
    tourism = tourism.select(
        'lat', 'lon',
        functions.from_unixtime(tourism['unix_time']).alias('timestamp'),
        'tourism', 'name', 'tags'
    )
    tourism = tourism.cache()
    tourism.write.json(output, mode='overwrite', compression='gzip')
    tourism.write.parquet(output + '-parquet', mode='overwrite', compression='lz4')
